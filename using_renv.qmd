...
## Scenario 1: First-time use of `renv` in a project {.compact}
...
*Key idea:* `renv.lock` is the single information source

*Note:* The scan may not catch all dependencies.


## Scenario 2: Start of work in a project that contains `renv.lock` {.compact}
...
**Note**: Sometimes renv::restore() does not install all the needed packages. It is a good practice to check ./renv/library folder to inspect after running the command.
...
- Advantages of using module-level renv.lock
  - renv::init() may not catch all dependencies. It is tedious to get a complete renv.lock by trial-and-error. Thus, it is better to avoid making renv.lock for each project
